...
public class DateCategoryFilter extends DateFilter {
    ...
    public boolean accepts(Event event) {
        ...
        boolean categoryMatches = this.category.primary().equalsIgnoreCase(eventCategory.primary());

        if (!this.category.secondary().isBlank()) {
            categoryMatches = categoryMatches && this.category.secondary().equalsIgnoreCase(eventCategory.secondary());
        }
